...
public class CustomFontCreatorPlugin {
	[MenuItem("Tools/Convert To Unity Font")][CanEditMultipleObjects]
...
//		TextAsset fntSetting = creator._FntSettings;
		...
		string materialPath = basePath + "/" + fileName + ".mat";
...
//			EditorUtility.SetDirty(fntMaterial);
		...
		string fntPath = basePath + "/" + fileName + ".fontsettings";
...
//			info.width = (float)bmInfo.advance;
			...
			info.minY = bmInfo.offsetY - (int)(bmInfo.height * 0.5f);
			info.maxX = info.minX + bmInfo.width;
			info.maxY = info.minY + bmInfo.height;
